...
#total of two datasets
    ...
    #populating the final demand emissions database
        ...
        #print("shape of h:")
        #print(h.shape)
        f = h5py.File('/home/sidney/experiments/%s_proper.hdf5' % year,'w')
    ...
    #iterating over import region
        ...
        def myfunc(j):
            ...
            f2 = h5py.File('/home/sidney/experiments/{0}_{1}_proper.hdf5'.format(year, j) ,'w')
            temperal = ('Footprint_dset_%s' % str(j))
            print(temperal)
            temperal = f2.create_dataset('Footprint', (nt, nr, ns, nr, ns, nr), dtype = 'f', compression="gzip", compression_opts=9 )

            temperal[i,:,:,:,:,j] = foot_tmp
        for j in range(1):
