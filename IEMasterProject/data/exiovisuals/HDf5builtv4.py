...
#total of two datasets
    ...
    #read stuff from matlab binary
        ...
        mat = scipy.io.loadmat(path+'sys_leo_%s.mat' % x) #%insert i in name
    ...
    #iterating over import region
        ...
        for j in range(49):
            ...
            #subprocess.call(['h5copy', '-i', '{0}','-s', '/Footprint', '-o', '{1}_combined.hdf5', '-d', '/region{2}'.format(path+str(x)+"_region"+str(j)+"_Footprint.hdf5",str(x), str(j))])
