...
class FancyTreeWidget(Widget):
    ...
    def value_from_datadict(self, data, files, name):
        if isinstance(data, (MultiValueDict, dict.update(data))):
    ...
    def render(self, name, value, attrs=None, choices=()):
      ...
      $("#%(id)s").fancytree("getRootNode").visit(function(node){
        node.toggleSelected();
      });
      return false;
    });
    $("#btnDeselectAll").click(function(){
      $("#%(id)s").fancytree("getTree").visit(function(node){
    ...
    class Media:
        css = {
            'all': ('fancytree/skin-win8/ui.fancytree.css',)
        }
        js = (
